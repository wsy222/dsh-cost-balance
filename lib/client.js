...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 逐笔消耗：从节点提取 [发生时刻ms, 未命中, 缓存读, 缓存写, 输出, 模型]。
     *  宿主优先按会话日志计费（完整逐笔），这份客户端数据只作回退，
     *  因此长会话重开后聊天树只物化了一部分节点时也不会算错。 */
    function collectSteps(nodes) {
      ...
      for (const node of nodes) {
        ...
        const uncached = Number(usage.uncachedInputTokens ?? usage.inputTokens ?? 0) || 0
        ...
        const timing = node.timing
        const at = timing !== void 0 && timing !== null && typeof timing.completedTime === 'number'
          ? timing.completedTime
          : (typeof node.time === 'number' ? node.time : Date.now())
        const route = usage.route ?? (Array.isArray(usage.routes) ? usage.routes[0] : void 0)
        const model = route !== void 0 && route !== null && typeof route.model === 'string' ? route.model : ''
        steps.push([at, uncached, cacheRead, cacheWrite, output, model])
      }
      return steps
    }
    function fetchReadout(sessionId, steps, usage) {
      const body = {
        sessionId,
        steps,
        usage: {
    ...
    function StatsPill(props) {
      ...
      const steps = React.useMemo(() => collectSteps(settledNodes), [settledNodes])
      ...
      // sessionId 让宿主能直接读完整会话日志计费（见 Host half 的 sessionLogCost）。
      const sessionId = props.sessionId
      const refresh = React.useCallback(() => {
        if (usage === void 0) return
        fetchReadout(sessionId, steps, usage).then((result) => {
          setReadout(result !== null && typeof result === 'object' ? result : null)
        }).catch(() => {
          setReadout(null)
        })
      }, [sessionId, usageKey, steps])
      React.useEffect(() => {
        refresh()
      }, [refresh])
      ...
      if (readout !== null) {
        ...
        // source=session-log 为宿主读完整会话日志的权威值；其余是回退估算，显式标注避免误信。
        if (cost !== null) {
          const band = readout.band === 'peak' ? '峰' : readout.band === 'offpeak' ? '谷' : ''
          const estimated = readout.source !== void 0 && readout.source !== 'session-log'
          rows.push(['花费', '¥' + formatCost(cost) + (band !== '' ? '(' + band + ')' : '') + (estimated ? '(估算)' : '')])
        }
