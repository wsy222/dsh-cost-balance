...
export function apply(ctx, config = {}) {
  ...
  /** 按当前北京时间返回生效价格：峰谷切换前用旧表；切换后高峰取 peakPrices、空闲半价；
   *  2026-08-23 起周末（周六/周日）全天按低谷价（高峰半价），不再区分峰谷。 */
  const priceOf = (model) => {
    ...
    if (weekendOffpeak) return { cacheHit: p.cacheHit / 2, cacheMiss: p.cacheMiss / 2, output: p.output / 2 }
