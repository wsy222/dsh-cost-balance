...
export function apply(ctx, config = {}) {
  ...
  /** 按当前北京时间返回生效价格与价格段：峰谷切换前 flat（无峰谷概念）；
  ...
  const priceOf = (model) => {
    ...
    if (beijingNow < PEAK_CHANGE_AT_BEIJING) return { price: p, band: 'flat' }
  ...
  const computeCost = (usage) => {
    ...
    const { price: p, band } = priceOf(model)
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const { cost, model, band } = computeCost(usage)
      const balance = await fetchBalance(force)
      sendJson(res, 200, { cost, model, band, balance })
    },
  }))
}
